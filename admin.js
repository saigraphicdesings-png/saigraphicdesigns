...
list.addEventListener("click",async e=>{const edit=e.target.closest("[data-edit]"),home=e.target.closest("[data-home]"),toggle=e.target.closest("[data-toggle]"),remove=e.target.closest("[data-delete]");if(remove){const p=products.find(x=>x.id===remove.dataset.delete);if(!p||!await confirmProductDelete(p))return;try{await api("/api/admin/products/"+encodeURIComponent(p.id),{method:"DELETE"});message.style.color="#047857";message.textContent="Product deleted.";await loadProducts()}catch(err){message.style.color="#dc2626";message.textContent=err.message}return}if(edit){fill(products.find(p=>p.id===edit.dataset.edit));return}if(home){const p=products.find(x=>x.id===home.dataset.home);if(!p||!p.active)return;try{await save({...p,showOnHome:!p.showOnHome});message.style.color="#047857";message.textContent=p.showOnHome?"Removed from homepage.":"Added to homepage.";await loadProducts()}catch(err){message.style.color="#dc2626";message.textContent=err.message}return}if(toggle){const p=products.find(x=>x.id===toggle.dataset.toggle);if(!p)return;try{await save({...p,active:!p.active});await loadProducts()}catch(err){message.textContent=err.message}}})
function fill(p){if(!p)return;$("formTitle").textContent="Edit Bundle";$("originalId").value=p.id;$("productId").value=p.id;$("productName").value=p.name;$("productPrice").value=p.price;$("productTier").value=p.tier||((Number(p.price)===0)?"free":(Number(p.price)>=499?"premium":"standard"));$("designCount").value=p.designCount||0;$("sortOrder").value=p.sort_order||0;$("productCategory").value=p.category;$("productType").value=p.type;Array.from($("productFormats").options).forEach(option=>{option.selected=(p.formats||[]).map(String).map(x=>x.toLowerCase()).includes(option.value)});window.bundleImages?.set(p.images||[]);$("productDescription").value=p.description||"";$("downloadUrl").value=p.downloadUrl||"";$("productActive").checked=Boolean(p.active);$("showOnHome").checked=Boolean(p.showOnHome);form.scrollIntoView({behavior:"smooth",block:"start"})}
function reset(){form.reset();window.bundleImages?.set([]);$("originalId").value="";$("productPrice").value="0";$("productTier").value="free";$("designCount").value="0";$("sortOrder").value="0";$("productActive").checked=true;$("showOnHome").checked=false;$("formTitle").textContent="Add Bundle";message.textContent=""}
function payload(){const originalId=$("originalId").value.trim();return{id:$("productId").value.trim(),originalId,name:$("productName").value.trim(),price:Number($("productPrice").value)||0,tier:$("productTier").value,designCount:Number($("designCount").value)||0,sort_order:Number($("sortOrder").value)||0,category:$("productCategory").value.trim(),type:$("productType").value.trim(),formats:Array.from($("productFormats").selectedOptions).map(option=>option.value),images:window.bundleImages?.get()||[],description:$("productDescription").value.trim(),downloadUrl:$("downloadUrl").value.trim(),active:$("productActive").checked,showOnHome:$("showOnHome").checked,isKeyProduct:products.find(product=>product.id===originalId)?.isKeyProduct===true}}
...
form.addEventListener("submit",async e=>{e.preventDefault();if(window.bundleImages?.busy()){message.textContent="Wait for images to finish uploading.";return}const draft=payload();if(!/\bbundles?\b/i.test(draft.name)){message.textContent="Bundle name must contain Bundle.";return}if(!draft.images.length){message.textContent="Add a bundle thumbnail.";return}message.textContent="Saving…";try{await save(draft);message.style.color="#047857";message.textContent="Bundle saved successfully.";reset();await loadProducts()}catch(err){message.style.color="#dc2626";message.textContent=err.message}})
...
$("productPrice").addEventListener("input",()=>{const price=Number($("productPrice").value);if(price===0)$("productTier").value="free";else if($("productTier").value==="free")$("productTier").value="standard"});$("resetBtn").addEventListener("click",reset);$("refreshBtn").addEventListener("click",loadProducts);$("logoutBtn").addEventListener("click",logout);$("productSearch").addEventListener("input",render);
