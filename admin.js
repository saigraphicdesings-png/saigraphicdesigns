...
function payload(){const originalId=$("originalId").value.trim();return{id:$("productId").value.trim(),originalId,name:$("productName").value.trim(),price:Number($("productPrice").value)||0,sort_order:Number($("sortOrder").value)||0,category:$("productCategory").value.trim(),type:$("productType").value.trim(),formats:Array.from($("productFormats").selectedOptions).map(option=>option.value),images:window.bundleImages?.get()||[],description:$("productDescription").value.trim(),downloadUrl:$("downloadUrl").value.trim(),active:$("productActive").checked,showOnHome:$("showOnHome").checked,isKeyProduct:products.find(product=>product.id===originalId)?.isKeyProduct===true}}
