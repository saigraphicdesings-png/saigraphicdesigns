...
function fill(p){if(!p)return;$("formTitle").textContent="Edit Product";$("originalId").value=p.id;$("productId").value=p.id;$("productName").value=p.name;$("productPrice").value=p.price;$("sortOrder").value=p.sort_order||0;$("productCategory").value=p.category;$("productType").value=p.type;Array.from($("productFormats").options).forEach(option=>{option.selected=(p.formats||[]).map(String).map(x=>x.toLowerCase()).includes(option.value)});$("productImages").value=(p.images||[]).join("\n");$("productDescription").value=p.description||"";$("downloadUrl").value=p.downloadUrl||"";$("productActive").checked=Boolean(p.active);$("showOnHome").checked=Boolean(p.showOnHome);form.scrollIntoView({behavior:"smooth",block:"start"})}
function reset(){form.reset();$("originalId").value="";$("productPrice").value="0";$("sortOrder").value="0";$("productActive").checked=true;$("showOnHome").checked=false;$("formTitle").textContent="Add Product";message.textContent=""}
function payload(){const originalId=$("originalId").value.trim();return{id:$("productId").value.trim(),originalId,name:$("productName").value.trim(),price:Number($("productPrice").value)||0,sort_order:Number($("sortOrder").value)||0,category:$("productCategory").value.trim(),type:$("productType").value.trim(),formats:Array.from($("productFormats").selectedOptions).map(option=>option.value),images:$("productImages").value.split(/\r?\n/).map(x=>x.trim()).filter(Boolean),description:$("productDescription").value.trim(),downloadUrl:$("downloadUrl").value.trim(),active:$("productActive").checked,showOnHome:$("showOnHome").checked,isKeyProduct:products.find(product=>product.id===originalId)?.isKeyProduct===true}}
...
$("saveKeyProduct").addEventListener("click",async()=>{
  ...
  finally{button.disabled=false;button.textContent="Set as Key Product"}
});
