...
function validateBlog(input) {
  ...
  const imageUrls=(Array.isArray(input.imageUrls)?input.imageUrls:[]).map(x=>String(x||"").trim()).filter(x=>/^https?:\/\//i.test(x)).slice(0,4);
  const allowedBlocks = new Set(["paragraph", "heading", "bullets", "numbers", "image", "quote", "faq"]);
  const blocks = Array.isArray(input.blocks) ? input.blocks.slice(0, 120).map((item) => ({ type: allowedBlocks.has(item?.type) ? item.type : "paragraph", text: String(item?.text || "").trim().slice(0, 5000), level: [2, 3, 4].includes(Number(item?.level)) ? Number(item.level) : 2, url: /^https?:\/\//i.test(String(item?.url || "").trim()) ? String(item.url).trim().slice(0, 2000) : "", alt: String(item?.alt || "").trim().slice(0, 250), items: Array.isArray(item?.items) ? item.items.map(x => String(x || "").trim().slice(0, 500)).filter(Boolean).slice(0, 80) : [], question: String(item?.question || "").trim().slice(0, 250), answer: String(item?.answer || "").trim().slice(0, 1200) })).filter((item) => item.text || item.url || item.items.length || (item.question && item.answer)) : [];
