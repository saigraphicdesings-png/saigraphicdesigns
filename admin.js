...
function fill(p){if(!p)return;$("formTitle").textContent="Edit Bundle";$("originalId").value=p.id;$("productId").value=p.id;$("productName").value=p.name;$("productPrice").value=p.price;$("sortOrder").value=p.sort_order||0;$("productCategory").value=p.category;$("productType").value=p.type;Array.from($("productFormats").options).forEach(option=>{option.selected=(p.formats||[]).map(String).map(x=>x.toLowerCase()).includes(option.value)});window.bundleImages?.set(p.images||[]);$("productDescription").value=p.description||"";$("articleTitle").value=p.articleTitle||"";$("articleContent").value=p.articleContent||"";$("articleFaqs").value=(p.articleFaqs||[]).map(item=>item.question+" | "+item.answer).join("\n");$("downloadUrl").value=p.downloadUrl||"";$("productActive").checked=Boolean(p.active);$("showOnHome").checked=Boolean(p.showOnHome);form.scrollIntoView({behavior:"smooth",block:"start"})}
function reset(){form.reset();window.bundleImages?.set([]);$("originalId").value="";$("productPrice").value="0";$("sortOrder").value="0";$("productActive").checked=true;$("showOnHome").checked=false;$("formTitle").textContent="Add Bundle";message.textContent=""}
function payload(){const originalId=$("originalId").value.trim();return{id:$("productId").value.trim(),originalId,name:$("productName").value.trim(),price:Number($("productPrice").value)||0,sort_order:Number($("sortOrder").value)||0,category:$("productCategory").value.trim(),type:$("productType").value.trim(),formats:Array.from($("productFormats").selectedOptions).map(option=>option.value),images:window.bundleImages?.get()||[],description:$("productDescription").value.trim(),articleTitle:$("articleTitle").value.trim(),articleContent:$("articleContent").value.trim(),articleFaqs:$("articleFaqs").value.split("\n").filter(line=>line.trim()).map(line=>{const split=line.indexOf("|");return{question:split<0?"":line.slice(0,split).trim(),answer:split<0?"":line.slice(split+1).trim()}}),downloadUrl:$("downloadUrl").value.trim(),active:$("productActive").checked,showOnHome:$("showOnHome").checked,isKeyProduct:products.find(product=>product.id===originalId)?.isKeyProduct===true}}
