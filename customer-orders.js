(function () {
  ...
  function render(el, orders) {
    ...
    el.innerHTML = '<div class="orders-head"><div><span class="account-kicker">MY ORDERS</span><h3>Order history</h3></div><button class="orders-refresh" type="button">Refresh</button></div>' + orders.map(function (o) {
      var items = (o.items || []).map(function (i) { return '<li><span>' + esc(i.productName) + ' x ' + esc(i.qty) + '</span><strong>' + money(i.lineTotal) + '</strong>' + (o.status === 'approved' ? '<button class="order-download" type="button" data-paid-download="' + esc(i.productId) + '">Open Drive Link</button>' : '') + '</li>'; }).join("");
      ...
      return '<article class="order-card"><div class="order-top"><div><strong>Order ' + esc(String(o.id).slice(0, 8).toUpperCase()) + '</strong><small>' + date(o.createdAt) + '</small></div><span class="order-status status-' + esc(o.status) + '">' + label + '</span></div><ul>' + items + '</ul><div class="order-foot"><span>Total <strong>' + money(o.amount) + '</strong></span><span>' + (o.kind === 'whatsapp' ? 'WhatsApp request: <strong>' + esc(String(o.id).slice(0, 8).toUpperCase()) : 'UTR: <strong>' + esc(o.utr || '-')) + '</strong></span></div>' + note + '</article>';
    }).join("");
