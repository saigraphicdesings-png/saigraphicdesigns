...
list.addEventListener("click",async e=>{const edit=e.target.closest("[data-edit]"),toggle=e.target.closest("[data-toggle]"),remove=e.target.closest("[data-delete]");if(remove){const p=products.find(x=>x.id===remove.dataset.delete);if(!p||!await confirmProductDelete(p))return;try{await api("/api/admin/products/"+encodeURIComponent(p.id),{method:"DELETE"});message.style.color="#047857";message.textContent="Product deleted.";await loadProducts()}catch(err){message.style.color="#dc2626";message.textContent=err.message}return}if(edit){fill(products.find(p=>p.id===edit.dataset.edit));return}if(toggle){const p=products.find(x=>x.id===toggle.dataset.toggle);if(!p)return;try{await save({...p,active:!p.active});await loadProducts()}catch(err){message.textContent=err.message}}})
